...
boost::container::map<int, std::string>* getMapList(std::string* str, boost::container::vector<int>* list) {
	if (!str || !list) {
		return nullptr;
	}

	boost::container::map<int, std::string>* mapList = new boost::container::map<int, std::string>();
